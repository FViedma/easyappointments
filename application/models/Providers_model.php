...
class Providers_model extends EA_Model
{
    ...
    public function get_available_providers()
    {
        ...
        $providers = [];
        foreach ($services as $service) {
            $next_provider = $this->get_next_provider($service);
            // Get provider records from database.
            $this->db
                ->select('users.*')
                ->from('users')
                ->join('roles', 'roles.id = users.id_roles', 'inner')
                ->where(array('roles.slug' => DB_SLUG_PROVIDER, 'users.id' => $next_provider['id_users']));
            $provider = $this->db->get()->row_array();
    ...
    public function get_next_provider($service)
    {
        ...
        $providers_position = array_values($providers);
        if (end($providers_position)['id_users'] === $current_provider) {
            $provider = $providers_position[0];
            $this->db->where('id', $service['id']);
            $this->db->update('services', array('current_provider' => $provider['id_users']));
        } else {
            foreach ($providers_position as $prov) {
                if ($current_provider < $prov['id_users'] || $service['current_provider'] == 0) {
                    $provider = $prov;
                    $this->db->where('id', $service['id']);
                    $this->db->update('services', array('current_provider' => $prov['id_users']));
                    break;
                }
            }
        }
